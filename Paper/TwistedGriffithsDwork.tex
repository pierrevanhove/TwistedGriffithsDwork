...
all necessary blow-ups (see~\cite{bek,Brown:2009ta,Bloch:2016izu,muller2014picard}.) This is in general a nontrivial task \lNote{expand}.  The    differentiation of $\Omega_\Gamma^\epsilon$ 
...
contributions.
 ...
 $c_{a_1,\dots,a_i-1,\dots,a_r}(\underline z) $ with $1\leq i\leq r$ thus fully determining the PF operator. 
...
%below.
 Let us comment on how to determine the order of the PF operators. An upper bound on the order can be computed
 from the number of master integrals as
 described
in~\cite{Bitoun:2017nre,Lee:2013hzt} (see~\cite{Agostini:2022cgv} for
summary of different equivalent way of computing the number of master integrals). On the other hand, determining the minimal order is a difficult question. Our pragmatical approach is to start from the small order until the system \eqref{e:sysCFU} has a solution, which is the spirit of the GD reduction applied to Feynman integrals in \cite{Muller-Stach:2011qkg}.  

%\pvnote{update the discussion and mention the work of %Muller-Stach/Weinzierl}

%By iterating this procedure the algorithm determines the coefficients
%in~\eqref{e:PFOmegaGeneric}. 
...
%--------------------------------------------------------------------
\section{Minimal order Picard-Fuchs operators in general dimensions}
\label{sec:results}
...
section is of \emph{minimal} order or not.  By minimal we mean an
