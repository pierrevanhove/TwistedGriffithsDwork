...
This is possible
because the Feynman integral are holonomic function of its physical parameters \cite{Kashiwara:1977nf}. This means that Feynman integrals satisfy systems of  (inhomogeneous) partial
differential equations with respect to its physical parameters
$ \{m_1^2,\dots,m_n^2,s_{ij}\}$, which can be seen as a D-module.  Thus, differential equations can be obtained by deriving the annihilators  of  $\Omega_\Gamma^\epsilon$. Let 
$\underline
z:=\{z_1,\dots,z_r\} \in \vec m \cup \vec s$. We seek 
...
parameters but they are independent of $x_1,\dots,x_n$.  %This is possible
...
all necessary blow-ups (see~\cite{bek,Brown:2009ta,Bloch:2016izu,muller2014picard}.) The    differentiation of $\Omega_\Gamma$ 
...
$  P^{(a_1,\dots,a_r)}(\underline x)$ is a
 ...
 the edge variables $\underline x:=\{x_1,\dots, x_n\}$ and the
  ...
  coefficient $c_{a_1,\dots,a_r}(\underline z)$.  A method for doing this in the
...
The starting point of the algorithm is the reduction of $P^{(a_1,\dots,a_r)}(\underline x)$ in the Jacobian ideal of $\textbf{F}_\Gamma$ 
 \begin{equation}
...
This expression involves the $\vec
    ...
    new pole in $\mathbf{U}_\Gamma$. We then perform a second reduction by demanding
    \begin{equation}
      \label{e:RedU}
      \vec
...
We start from the number of derivative $a$ \lNote{$N$?} equal to the
...
$c_{a_1,\dots,a_i-1,\dots,a_r}(\underline z) $ with $1\leq i\leq r$
