...
We demonstrate the applicability of this algorithm by providing the inhomogeneous differential equations for the dimensionally regularized multiloop two-point sunset integrals: up to the 20th loop order for the all equal mass case, the generic mass case at the two-loop and the three-loop order. Additionally, we present the Picard-Fuchs operator for the two-loop three-point ice-cream cone graph.


We extend our algorithm to apply it to the dimensionally and analytically regulated Witten diagrams for cosmological correlators of conformally coupled $\phi^4$
...
in~\eqref{e:OmegaDimRegIntro} and~\eqref{e:OmegaAnRegIntro} and are integers,
...
\noindent{\bf Dimensional regularisation:} One can regulate the integral by using the dimensional regularisation
...
\noindent{\bf Analytic regularisation:} Another regularisation is the
...
\begin{equation}
	\omega_{\IceCream,  2-2\epsilon}(t)= {\textbf{U}_{(2,1,1)}\over
...
\begin{align}
	...
	\textbf{V}_{\IceCream}&:=p_2^2y_1y_2(z+x_1)+zx_1(p_1^2y_1+p_2^2y_2),\\
	\nonumber  \textbf{F}_{\IceCream;D}(t)&:=\textbf{U}_{\IceCream}
	(\mu_1^2y_1+\mu_2^2y_2^2+m_1^2x_1+m_2^2z)-t \textbf{V}_{\IceCream;D}.
\end{align}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Cosmological correlators in de Sitter space}\label{e:AdS}

The evaluation of loop contributions to the 
 cosmological correlators for conformally coupled $\phi^4$  in  de
 Sitter can be organised  as dimensionally regulated flat space Feynman integrals in direct
space~\cite{Heckelbacher:2022fbx,Heckelbacher:2022hbq} or
analytically regulated in momentum space~\cite{Chowdhury:2023arc}.
Both these cases the powers of the propagators involve the regulator
and lead to parametric expressions in the class of the analytic
regularisation of Section~\ref{sec:PoleRedAn}.

%-------------------------------------------------------------------------
\subsection{The Cross Witten diagram in $AdS_4$}
 \begin{figure}[ht]
                \centering
...
%-------------------------------------------------------------------------
\subsection{The Witten ice-cream cone diagram}
...
\begin{equation}
  I_{\IceCream}= \int \frac{d^{4}L_{4}d^{4}L_{2}}{(L_2^2)^{1+\kappa}(L_4^2)^{1+\kappa}((L_4+L_2+Q)^2)^{1+\kappa}((L_4+L_2+\tilde Q)^2))^{1+\kappa} }  
\end{equation}
...
\begin{equation}
  \Omega_{\IceCream}^\kappa= {\pi^4\Gamma(4\kappa)\over
    \Gamma(1+\kappa)^4} \,{1\over \textbf{U}_{\IceCream}^{4} }
  \left(\textbf{U}_{\IceCream}^{4}\prod_{i=1}^4 x_i^\kappa\over \textbf{F}_{\IceCream}^{4}  \right)^\kappa\,\Omega_4^{(0)}\,,
\end{equation}
...
\begin{equation}
  ...
  \mathscr{L}_1&=\left(1-u
   -v\right) v\left(\partial\over\partial v\right)^{2} -2u
                 v{\partial\over\partial u}{\partial\over\partial v}-(3 \kappa+1)u {\partial\over\partial u}++(3 \kappa -3 \kappa 
  u-6 \kappa  v-v){\partial\over\partial v}-8 \kappa ^2,\cr
