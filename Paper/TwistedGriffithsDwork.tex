...
for various  two-loop graphs.

We apply, as well, our algorithm for deriving Gr\"obner basis of
partial differential operators for the dimensionally and analytically
regulated Witten diagrams   arising in the evaluation of the
correlation functions and cosmological correlators of conformally coupled $\phi^4$
  theory in four-dimensional de anti-de Sitter and de Sitter space respectively.
...
edge variables.   The Gel'fand-Kapranov-Zelevinski\u\i (GKZ) formalism provides
...
\end{equation}
A {\tt Mathematica} notebook with  example is accessible at \href{Example-Twisted-Griffiths-Dwork.nb}{Example-Twisted-Griffiths-Dwork.nb}
...
\end{align}
Notice that $\textbf{U}_{n-1}^n/ \textbf{F}_{n-1}(t)^{n-1}$ is a homogeneous
...
For special kinematic configurations leading to new singularities of
the Feynman integral, the signed Euler characteristic of the complement
of the graph singularities in $\mathbb P^{n-1}$ is smaller than
the generic one. These configuration below to the Euler discriminant
variety defined in~\cite{Fevola:2023kaw}, which is 
conjectured to contains  the singularities of the Feynman integral.

Unfortunately, in the case of special kinematic configurations that do not lead
to new singularities,  the order drop is not detected by  either the
