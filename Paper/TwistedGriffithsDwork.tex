...
various physical phenomena. \lNote{more motivation}
...
\pvnote{more here}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Griffiths-Dwork reduction in general dimensions}
...
%These
%work do not make a specific use of 
we will use
...
$ \{m_1^2,\dots,m_n^2,s_{ij}\}$, which can be seen as a D-module\lNote{define and GKZ?}.  Thus, differential equations can be obtained by deriving  annihilators  of  $\Omega_\Gamma^\epsilon$, i.e., partial differential operators that annihilate  the integrand by acting on the physical parameter and the edge variables. Let us consider $r$ parameters from the set of internal masses and independent kinematics, $\underline z:=\{z_1,\dots,z_r\} \in \vec m \cup \vec s $. 
%$\underline
%$.
 We seek 
...
The
  ...
  In the coming sections we will present an algorithm to derive these coefficients and the inhomogeneous term of the differential equation.
...
%$\widehat{dx_i}$
% means that the term is omitted in the wedge
%product and
the twisted vectors 
\begin{equation}
  \label{e:Gdef}
...
%$\underline x$. 
%
%Following the analysis by Griffiths~\cite{Griffiths_1969}  extending it for
%derivation
 %non polynomial but homogeneous function. \lNote{unclear}
We compute
\begin{multline}
...
c_{\mathsf{a}}(\underline x) \vec{x} \right) \, \cdot\vec\nabla \mathbf{U}_\Gamma =0\, ,\end{equation}
which are examples of syzygies of the Jacobian of $\mathbf U_\Gamma$. Using this reduction in eq.~\eqref{eq:Pred} leads to
% 
...
%get from~\eqref{e:PDE} to~\eqref{e:RedFinal},
...
\subsection{Algorithmic derivation of the differential equation}
...
%\pvnote{may be somewhere we can quite Gel'fand}\lNote{we said this before}
%
%
...
%that needs to be
%determined. 
The polynomial $ P_{N(\Gamma,\epsilon)-1}(\underline x) $
...
$M_{N(\Gamma,\epsilon)-1}(x)$. One iterates the reduction until the power of the pole is $n-L-1$ so
that
...
contributions.
 ...
 $c_{a_1,\dots,a_i-1,\dots,a_r}(\underline z) $ with $1\leq i\leq r$ thus fully determining the Picard-Fuchs operator. 
 

%up-to ambiguities by shifting with syzygies of  Jacobian ideal of
 %$\textbf{F}_\Gamma$ \lNote{up to shifts ...}.  
 
% This freedom plays an important role in the case
...
differential operator with numerical coefficients
...
In Section~\ref{sec:griff-dwork-reduct}, we have presented an algorithm for deriving
the inhomogeneous differential equations satisfied by Feynman
integrals. The algorithm solves for the coefficients in the
differential equations with respect to the physical parameters and
determines the inhomogeneous term of the differential
equation. \pvnote{Give the key equation numbers} 
...
There are two cases in which the order is  reduced \pvnote{ more
...
The phenomenon of order reduction can be seen by computing  the
factorisation of the differential operator.
The minimal order differential operator for  special mass or kinematic
configurations is a right factor of the general 
configuration differential operator.  It has been shown
in~\cite{Lairez:2022zkj}  how to \pvnote{XXX}  $L^{\rm
  generic}\to_{specialize} \tilde L= L_1 L_{\rm minimal}$
...
\pvnote{cite Weinzierl genus drop}
...
$\mathscr{L}_z^\epsilon$. The  $\epsilon$ affects the monodromy of differential equation around
...
%	doi:10.1007/s00220-013-1838-3
	...
	%73 citations counted in INSPIRE as of 07 Dec 2023
...
\bibitem{Bitoun:2017nre}
	...
	doi:10.1007/s11005-018-1114-8
	[arXiv:1712.09215 [hep-th]].
	%72 citations counted in INSPIRE as of 18 Dec 2023
...
doi:10.1007/s11005-010-0450-0
[arXiv:1004.4199 [hep-th]].
%112 citations counted in INSPIRE as of 18 Dec 2023
\end{thebibliography}
