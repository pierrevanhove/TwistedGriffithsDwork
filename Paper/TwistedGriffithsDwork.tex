...
%,  are  variables
% and $L$ is the number of loops of the graph.
%
...
converges,  and the (unique) value of the Feynman integral is  defined by analytic continuation~\cite{Speer}.\lNote{paragraph goes before}
%-----------------------------------------------------------------------
...
clearer the role of the twist from the non even integer space-time dimension.\lNote{sentence unclear}
...
\noindent{\bf Analytic regularisation:} In
analytic regularisation  one keeps fixed the space-time dimension
$D=2\delta-2\epsilon$ and instead  one considers the power of propagators as slightly shifted from integer values, that is  $\nu_i=\upnu_i
...
\end{equation}
%
The twists are the $\epsilon$ or $\kappa$-th powers of a homogeneous degree zero rational functions.
...
$  P^{(a_1,\dots,a_r)}(\underline x)$ is a
 ...
 the edge variables $\underline x:=\{x_1,\dots, x_n\}$. \lNote{defined before}
...
The
 ...
 In the coming section we present an algorithm to derive the
  ...
  equation~(\ref{e:PDEintFeynman}) in dimensional regularisation and analytic regularisation. \lNote{shorten, already said in intro}
 ...
 In Section~\ref{sec:deriv-diff-equat} we explain how to iteratively use these pole reductions for
  ...
  Feynman integral.\lNote{already summarized in introduction}
...
\end{equation}
which\lNote{whose} components are vectors of homogeneous degree $(\mathsf{a}-1)(L+1)+1-n$. 
...
%------------------------------------------------------------------------
\subsection{The massless box graph}\label{sec:box}
...
\begin{figure}[h]
	...
	\caption{Special two-point Kite. Dashed lines are massless
          propagators and solid lines are massive propagators.}\label{fig:kite1mass}
\end{figure}
We consider the two points  Kite graph with  3 massive propagators and
...
\begin{equation}
	\Omega_{\IceCream,  2-2\epsilon}(t)= {\textbf{U}_{(2,1,1)}\over
		(\textbf{F}_{(2,1,1);2-2\epsilon})^2} \left(\textbf{U}_{(2,1,1)}^3\over  (\textbf{F}_{(2,1,1);2-2\epsilon})^2\right)^{\epsilon}\,\Omega_4^{(0)},
\end{equation}
...
%		\filldraw [black] (2.5,-1.5) circle (2pt);
	...
	\caption{The roast chicken graph. Dashed lines are massless
          propagators and solid lines are massive propagators.}\label{fig:roastchicken}
\end{figure}
...
%		\filldraw [black] (-1.414,-1.414) circle (2pt);
		...
		\draw [black,very thick] (-1.414,-1.414) to (-2.25,-2.25);
	\end{tikzpicture}
	\caption{The planar and non-planar massless double-box graphs}\label{fig:doublebox}
\end{figure}
...
\pvnote{fix the drawing of the graph in the subscript}
\begin{multline}
