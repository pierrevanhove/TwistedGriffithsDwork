...
%, which the case for instance of  the two-loop
%sunset $L=2$ and $n=3$ case, 
  we apply the standard Griffiths-Dwork
 pole reduction as in~\cite{Bloch:2016izu}.
  Because  $\nu>L\delta$ the polar part arises
...
%
 %    
  ...
  \beta^{(a_1,\dots,a_r)}=  \sum_{1\leq i<j\leq n} {x_i
    C_{\mathsf{a}}^j  (\underline x)-x_j
    C_{\mathsf{a}}^i(\underline x)\over  (\mathsf{a}+\nu-L\delta-1)\textbf{F}_\Gamma^{\mathsf{a}+\nu-L\delta-1}}\cr\times
...
product,   so that
   \begin{equation}
     d\beta^{(a_1,\dots,a_r)}= {  \vec   C_{\mathsf{a}}
     (\underline x)
\cdot    \vec \nabla\textbf{F}_\Gamma\over
     \textbf{F}_\Gamma^{\mathsf{a}+\nu-L\delta}}\Omega_0^{(n)}-{
...
\begin{multline}
  ...
  ={M^{(a_1,\dots,a_r)}(\underline x)+\sum_{i=1}^r
    c_{a_1,\dots,a_i-1,\dots,a_r}(\vec m,\vec s)    P^{(a_1,\dots,a_i-1,\dots,a_r)}(\underline x) \over \textbf{F}_\Gamma^{\mathsf{a}+\nu-L\delta-1}} \Omega_\Gamma^0\cr+d\beta^{(a_1,\dots,a_r)} ,
\end{multline}
where we have introduced $M^{(a_1,\dots,a_r)}(\underline x):=\vec\nabla C_{\mathsf{a}} (\underline x)/ (\sum_i a_i+\nu-L\delta-1)$.
...
\begin{equation}\label{e:Redstep2}
    ...
    =\vec
    C_{\mathsf{a}-1}(\underline x)\cdot\vec \nabla\textbf{F}_\Gamma
\end{equation}
%
...
obtain \lNote{sign is here, I think}
\begin{multline}
...
partial differential case $a=\mathsf{a}$) such that the system has
positive rank.
...
The goal of the reduction is then to iteratively construct the matrix $M_{(a)}$ at for each $a$ starting from the highest degree. If we call the function that reduces a given numerator by $f_q(M_q, P)$, where $q$ is the degree under consideration and $P$ is the polynomial in \eqref{derivativeOmega}. The iteration can be written as follows\lNote{notation needs to improve here}
%
\begin{align}
	M_{k-i+1}=f_{k+1-i}(M_{k+2-i},P_i), \quad i\ge 2, \quad k-i+1 \ge 1\, ,
\end{align}
where $k$ is the starting degree. If the starting degree is too low, the system might not have a solution but we have found that in the cases where there is a solution, the iteration terminates.  This has also been noted previously in \cite{Muller-Stach:2012tgj}, where the authors applied a similar approach using Griffiths ideas. \lNote{rephrase}
